...
class Multiply(_Merge):
    ...
    def _merge_function(self, inputs):
        ...
        for i in range(1, len(inputs)):
            output = np.add(np.multiply(output * 0.5 + 0.5 - AAA - epsilon,  (inputs[1] * 0.5 + 0.5) * alpha + beta), AAA)
            output = (output * 2 - 1)
        return output
...
class Divide(_Merge):
    ...
    def build(self, input_shape):
        ...
        if len(input_shape) != 2:
            raise ValueError('A `Divide` layer should be called '
                             'on exactly 2 inputs')
    ...
    def _merge_function(self, inputs):
        if len(inputs) != 2:
            raise ValueError('A `Divide` layer should be called '
                             'on exactly 2 inputs')
        output = np.true_divide(np.subtract(inputs[0] * 0.5 + 0.5, AAA), (inputs[1] * 0.5 + 0.5) * alpha + beta) + AAA + epsilon
        output = (output * 2 - 1)
        return output
